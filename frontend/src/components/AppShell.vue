...
type NavIcon = "workbench" | "analytics" | "admin";
type NavEntry = {
    label: string;
    to: string;
    icon: NavIcon;
...
const primaryNav = computed<NavEntry[]>(() => [
    {
        ...
        to: "/workbench",
        icon: "workbench",
        permissions: ["workbench.api_key.read_own"],
    },
    {
        label: "Analytics",
        to: "/analytics",
        icon: "analytics",
        permissions: ["analytics.personal.read"],
    },
    {
        label: "Admin",
        to: "/admin",
...
const visibleNav = computed(() =>
    primaryNav.value.filter(
        (n) => !n.permissions || auth.hasAnyPermission(n.permissions),
    ),
);
        ...
        <aside class="nav">
            ...
            <div class="nav-footer">
                <div class="identity">
                    <div class="avatar">
                        {{
                            auth.user?.enterprise_email?.[0]?.toUpperCase() ??
                            "?"
                        }}
                    </div>
                    <div class="identity-text">
                        <div class="email">
                            {{ auth.user?.enterprise_email }}
                        </div>
        ...
        <main class="content">
            <header class="toolbar">
                <div class="toolbar-left">
                    <ToolbarIconButton
                        :label="shell.navCollapsed ? 'Expand' : 'Collapse'"
                        @click="shell.toggleNav()"
                    >
                        <svg
                            width="18"
                            height="18"
