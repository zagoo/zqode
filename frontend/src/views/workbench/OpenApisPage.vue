...
<template>
  <div class="page">
    <header>
      ...
      <p class="muted">Use these endpoints with OpenAI- or Anthropic-compatible clients.</p>
    </header>
    <div class="grid">
      <article v-for="o in apis" :key="o.openapi_id" class="card">
        ...
        <div class="url">
          ...
          <button class="btn ghost sm" @click="copy(o.gateway_url)">Copy</button>
        </div>
        <p v-if="o.usage_description" class="muted">{{ o.usage_description }}</p>
      </article>
...
.grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: var(--s-lg); }
@media (max-width: 720px) { .grid { grid-template-columns: 1fr; } }
.card header { display: flex; flex-direction: column; gap: var(--s-xs); }
.url { display: flex; align-items: center; gap: var(--s-xs); }
.url code { flex: 1; overflow-x: auto; background: var(--c-canvas); padding: 6px 10px; border-radius: var(--r-md); border: 1px solid var(--c-hairline); }
